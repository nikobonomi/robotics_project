...
class DifferentialRobot:
    ...
    def simulate_dt(self, dt):
        ...
        drive robot with wheel speeds linear_vel and angular_vel and wheelbase length """

        # if we are moving straight, R is at the infinity and we handle this case separately
        if np.isclose(self.linear_vel, self.angular_vel):
            transformation_matrix = self.mk_tr((self.linear_vel + self.angular_vel) / 2 * dt, 0)  # note we translate along x ()
        else:
            omega = (self.angular_vel - self.linear_vel) / (2 * self.length)  # angular speed of the robot frame
            r = self.length * (self.angular_vel + self.linear_vel) / (self.angular_vel - self.linear_vel)
